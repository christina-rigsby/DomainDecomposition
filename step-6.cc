    ...
    //} else if (subdomain == 1) {
        ...
        for (const auto &cell : triangulation.cell_iterators())
            for (const auto &face : cell->face_iterators()) {
                ...
                //Set boundary_id to 8 along gamma8, the left-most portion of the remaining portion of
    ...
    //} else if (subdomain == 2) {
        ...
        for (const auto &cell : triangulation.cell_iterators())
            for (const auto &face : cell->face_iterators()) {
                ...
                //Set boundary_id to 5 along gamma5, the left-most half of the remaining portion of
    ...
    //} else if (subdomain == 3) {
        ...
        for (const auto &cell : triangulation.cell_iterators())
            for (const auto &face : cell->face_iterators()) {
                ...
                //Set boundary_id to 6 along gamma6, the upper-most half of the remaining portion of
                ...
                //Set boundary_id to 5 along gamma5, the remaining portion
                ...
                //Set boundary_id to 6 along gamma6, the remaining portion
                // of subdomain3's top edge
                if ((std::fabs(center(dim - 1) - (0.125)) < 1e-12) &&
                    (center(0) >= 0))
                    face->set_boundary_id(6);
...
void Step6<dim>::output_results(const unsigned int cycle) const
{
...

    {
        ...
        std::ofstream output("res_add_center_asym_NOREF_solution-" + std::to_string(s * 100 + cycle) + ".vtu");
